...
public class UpdateEventValidator : AbstractValidator<UpdateEventRequest>
{
    public UpdateEventValidator()
    {
        ...
        // Cross-field rule: sum of tier quantities must not exceed TotalCapacity.
        // It is allowed to be less — the event manager may expand individual tiers later.
        RuleFor(x => x)
            .Must(x => x.PricingTiers != null &&
                       x.PricingTiers.Sum(t => t.Quantity) <= x.TotalCapacity)
            .WithMessage("The sum of all pricing tier quantities must not exceed the total capacity.")
